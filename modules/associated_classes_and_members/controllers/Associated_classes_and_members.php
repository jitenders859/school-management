<?php
class Associated_classes_and_members extends Trongate {

    private $associated_modules = ['classes', 'members'];
    private $assoc_module = 'associated_classes_and_members';
    private $relationshipType = 'one to one';
    private $bridgingTblRequired = 'true';

    function _get_identifier_column($module_name) {

        if ($module_name == 'classes') {
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'classes') {
            $data['associated_records_singular'] = 'Member ';
            $data['associated_records_plural'] = 'members';
        } else {
            $data['associated_records_singular'] = 'Class ';
            $data['associated_records_plural'] = 'classes';            
        }
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_classes_and_members.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_classes_and_members ON associated_module_name.id = associated_classes_and_members.associated_module_name_id
                WHERE
                    associated_classes_and_members.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_classes_and_members                ON '.$associated_module_name.'.id = associated_classes_and_members.'.$associated_module_name.'_id 
                WHERE associated_classes_and_members.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_classes_and_members.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_classes_and_members.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_classes_and_members.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'classes') {
            $associated_module_name = 'members';
        } else {
            $associated_module_name = 'classes';
        }
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'members') {
            $data['classes_id'] = $params['selectedValue'];
            $data['members_id'] = $params['update_id'];
        } else {
            $data['members_id'] = $params['selectedValue'];
            $data['classes_id'] = $params['update_id'];
        }
