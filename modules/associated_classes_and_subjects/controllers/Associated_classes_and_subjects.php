<?php
class Associated_classes_and_subjects extends Trongate {

    private $associated_modules = ['classes', 'subjects'];
    private $assoc_module = 'associated_classes_and_subjects';
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_classes_and_subjects.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_classes_and_subjects ON associated_module_name.id = associated_classes_and_subjects.associated_module_name_id
                WHERE
                    associated_classes_and_subjects.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_classes_and_subjects                ON '.$associated_module_name.'.id = associated_classes_and_subjects.'.$associated_module_name.'_id 
                WHERE associated_classes_and_subjects.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_classes_and_subjects.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_classes_and_subjects.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_classes_and_subjects.'.$calling_module_name.'_id IS NULL';
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'subjects') {
