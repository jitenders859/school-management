<?php
class Associated_video_lessons_and_video_resources extends Trongate {

    private $associated_modules = ['video_lessons', 'video_resources'];
    private $assoc_module = 'associated_video_lessons_and_video_resources';
    private $relationshipType = 'many to many';
    private $bridgingTblRequired = 'true';

    function _get_identifier_column($module_name) {

        if ($module_name == 'video_lessons') {
            $identifier_column = 'title';
        } else {
            $identifier_column = 'button_title';
        }
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'video_lessons') {
            $data['associated_records_singular'] = 'Video Resource ';
            $data['associated_records_plural'] = 'video resources';
        } else {
            $data['associated_records_singular'] = 'Video Lesson ';
            $data['associated_records_plural'] = 'video lessons';            
        }
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_video_lessons_and_video_resources.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_video_lessons_and_video_resources ON associated_module_name.id = associated_video_lessons_and_video_resources.associated_module_name_id
                WHERE
                    associated_video_lessons_and_video_resources.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_video_lessons_and_video_resources                ON '.$associated_module_name.'.id = associated_video_lessons_and_video_resources.'.$associated_module_name.'_id 
                WHERE associated_video_lessons_and_video_resources.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_video_lessons_and_video_resources.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_video_lessons_and_video_resources.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_video_lessons_and_video_resources.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'video_lessons') {
            $associated_module_name = 'video_resources';
        } else {
            $associated_module_name = 'video_lessons';
        }
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'video_resources') {
            $data['video_lessons_id'] = $params['selectedValue'];
            $data['video_resources_id'] = $params['update_id'];
        } else {
            $data['video_resources_id'] = $params['selectedValue'];
            $data['video_lessons_id'] = $params['update_id'];
        }
