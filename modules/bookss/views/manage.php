<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>bookss/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'book_name LIKE': '%' + searchPhrase + '%',        
      'OR subject LIKE': '%' + searchPhrase + '%',        
      'OR writter_name LIKE': '%' + searchPhrase + '%',        
      'OR id_no LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/bookss';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no books records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/bookss';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>bookss/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['book_name']}</td>
                        <td>${records[i]['subject']}</td>
                        <td>${records[i]['writter_name']}</td>
                        <td>${records[i]['id_no']}</td>
                        <td>${records[i]['published_date']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
