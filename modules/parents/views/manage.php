<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>parents/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'email_address LIKE': '%' + searchPhrase + '%',        
      'OR mobile_number LIKE': '%' + searchPhrase + '%',        
      'OR code LIKE': '%' + searchPhrase + '%',        
      'OR occupation LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/parents';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no parent records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/parents';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>parents/show/' + records[i]['id'];
      ...
      var newRow = `<tr>
                        ...
                        <td>${records[i]['occupation']}</td>
                        <td>${records[i]['child_id']}</td>
                        <td>${records[i]['member_id']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
