<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>schools/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {
      'OR school_name LIKE': '%' + searchPhrase + '%',
      'OR mobile_number LIKE': '%' + searchPhrase + '%',
      'OR telephone_number LIKE': '%' + searchPhrase + '%',
      'OR your_email_address LIKE': '%' + searchPhrase + '%',
      'OR address_line_1 LIKE': '%' + searchPhrase + '%',
      'OR city LIKE': '%' + searchPhrase + '%',
      'OR state__province__region LIKE': '%' + searchPhrase + '%',
      'OR zip__postal_code LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/schools';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no school records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/schools';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>schools/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['school_name']}</td>
                        <td>${records[i]['mobile_number']}</td>
                        <td>${records[i]['telephone_number']}</td>
                        <td>${records[i]['your_email_address']}</td>
                        <td>${records[i]['address_line_1']}</td>
                        <td>${records[i]['city']}</td>
                        <td>${records[i]['state__province__region']}</td>
                        <td>${records[i]['zip__postal_code']}</td>
                        <td>${records[i]['country']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
