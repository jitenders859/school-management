<?php
class Subject_types extends Trongate {
    ...
    function manage() {
        ...
        $data['total_rows'] = $this->model->count('subject_types');
        $data['record_name_plural'] = 'subject types';

        $data['headline'] = 'Manage Subject Types';
        $data['view_module'] = 'subject_types';
    ...
    function show() {
        ...
        if ($data == false) {
            ...
            $data['form_location'] = BASE_URL.'subject_types/submit/'.$update_id;
            $data['update_id'] = $update_id;
            $data['headline'] = 'Subject Type Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
    ...
    function create() {
        ...
        if ($update_id > 0) {
            $data['cancel_url'] = BASE_URL.'subject_types/show/'.$update_id;
            $data['btn_text'] = 'UPDATE SUBJECT TYPE DETAILS';
        } else {
            $data['cancel_url'] = BASE_URL.'subject_types/manage';
            $data['btn_text'] = 'CREATE SUBJECT TYPE RECORD';
        }
    ...
    function _get_page_headline($update_id) {
        //figure out what the page headline should be (on the subject_types/create page)
        if (!is_numeric($update_id)) {
            $headline = 'Create New Subject Type Record';
        } else {
            $headline = 'Update Subject Type Details';
        }
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if (is_numeric($update_id)) {
                    ...
                    $this->model->update($update_id, $data, 'subject_types');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'subject_types');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('subject_types/show/'.$update_id);
    ...
    function _get_data_from_db($update_id) {
        $subject_types = $this->model->get_where($update_id, 'subject_types');

        if ($subject_types == false) {
            $this->template('error_404');
            die();
        } else {
            $data['title'] = $subject_types->title;
            return $data;
        }
    }
