<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>subjects/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'subject_name LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/subjects';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no subject records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/subjects';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>subjects/show/' + records[i]['id'];
