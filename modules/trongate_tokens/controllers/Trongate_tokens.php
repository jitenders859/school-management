...
class Trongate_tokens extends Trongate {
    ...
    function _generate_token($data) {
        ...
        if(isset($data['school_id'])) {
            setcookie('school_id', $data['school_id'], $data['expiry_date'], '/');
        }
    ...
    function _destroy() {
        ...
        if (isset($_SESSION['school_id'])) {
            $tokens_to_delete[] = $_SESSION['school_id'];
            $_SESSION['school_id'] = 'x'; //fallback
            unset($_SESSION['school_id']);
        }

        if (isset($_COOKIE['school_id'])) {
            //destroy the school id  cookie
            $tokens_to_delete[] = $_COOKIE['school_id'];
            $past_date = time()-86400;
            setcookie('school_id', 'x', $past_date, '/');
        }
    ...
    function _attempt_get_school_id($token) {



        if (isset($_COOKIE['school_id'])) {
            $school_ids[] = $_COOKIE['school_id'];
        }

        if (isset($_SESSION['trongatetoken'])) {
            $school_ids[] = $_SESSION['school_id'];
        }
        ...
        if (!isset($school_ids)) {
            ...
            if(count($school_ids) > 1) {
                ...
                trongate_tokens
                JOIN members
                ON trongate_tokens.user_id = members.trongate_user_id WHERE trongate_tokens.token = :token";
