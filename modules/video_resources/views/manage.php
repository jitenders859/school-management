<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>video_resources/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'button_title LIKE': '%' + searchPhrase + '%',        
      'OR target_url LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/video_resources';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no video resource records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/video_resources';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>video_resources/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['button_title']}</td>
                        <td>${records[i]['target_url']}</td>
                        <td>${records[i]['downloadable']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
